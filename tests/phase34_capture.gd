...
func _init() -> void:
	...
	await shot("phase34_kaka_hook_aim",5)
	game._secondary_released()
	game._tick_kaka_hook_projectile(0.18)
	await shot("phase34_kaka_hook_flight",2)
	for i in range(40):
	...
	game.skill_q_cd=0.0
	game._cast_skill(0)
	await shot("phase34_bone_wall_preview",5)
	game._cast_skill(0)
	await shot("phase34_bone_wall_solid",3)
	game.skill_e_cd=0.0
	game._cast_skill(1)
	game._tick_kaka_rush_contacts()
	await shot("phase34_wall_push",8)
	await create_timer(0.34).timeout
	await shot("phase34_wall_endpoint_shatter",2)
	print("GODOT_PHASE34_CAPTURE_OK shots=6")
	quit()
