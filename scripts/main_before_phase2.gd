...
func _build_role_select() -> void:
	...
	for i in range(4):
		...
		button.custom_minimum_size = Vector2(245, 170)
...
func _update_hud() -> void:
	...
	status_label.text = "%s | Q %s %s | E %s %s" % [ROLE_NAMES[role_index], skills[0], qtxt, skills[1], etxt]
...
func _cast_skill(slot: int) -> void:
	...
	var mat := _mat(Color(color.r, color.g, color.b, 0.34), true)
	mat.emission_enabled = true
	mat.emission = color
	...
	tween.tween_property(pulse, "scale", target_scale, 0.48)
	...
	tween.tween_property(active_camera, "fov", base_fov, 0.26)
