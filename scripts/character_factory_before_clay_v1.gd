...
static func _build_bubble(root: Node3D) -> Dictionary:
	...
	var body := _sphere(root, "BubbleBody", Vector3(0, 0.96, 0), Vector3(0.88, 1.16, 0.78), cyan, 0.62, 0.95)
	var head := _sphere(root, "BubbleHead", Vector3(0, 1.58, -0.03), Vector3(0.92, 0.78, 0.82), pale, 0.50, 0.68)
...
static func _build_shroom(root: Node3D) -> Dictionary:
	...
	var body := _sphere(root, "ShroomBody", Vector3(0, 0.82, 0), Vector3(0.68, 0.92, 0.62), mint, 1.0, 0.08)
