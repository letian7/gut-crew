...
func _damage_enemy(e: CharacterBody3D, amount: float, stun := 0.0, pin := 0.0, impulse := Vector3.ZERO) -> void:
	...
	if e != mouse_target:
		...
		if combo_t > 0.0 and last_role >= 0 and last_role != role_index:
			...
			SkillVFX.spawn_sync(self, e.global_position + Vector3.UP * 0.12, chain, ROLE_COLORS[last_role], ROLE_COLORS[role_index])
		elif combo_t <= 0.0:
			chain = 1
...
func _tick_enemies(delta: float) -> void:
	for e in enemies:
		...
		var attack_pose: float = clampf(1.75 - e.global_position.distance_to(player.global_position), 0.0, 1.0)
