...
static func _hairball(root: Node3D) -> MeshInstance3D:
	...
	sphere(root,"Tongue",Vector3(0,0.47,-0.56),Vector3(0.22,0.12,0.18),pink)
	return body
static func _platelet(root: Node3D) -> MeshInstance3D:
	...
	var helmet := sphere(root,"HelmetDome",Vector3(0,1.20,0),Vector3(0.76,0.34,0.70),yellow)
	helmet.scale.y = 0.48
...
static func _parasite(root: Node3D) -> MeshInstance3D:
	...
	var body := sphere(root,"ParasiteHead",Vector3(0,0.82,-0.30),Vector3(0.78,0.76,0.72),green)
	for i in range(5):
		var z := 0.18 + float(i)*0.36
		sphere(root,"ParasiteSegment",Vector3(0,0.66,z),Vector3(0.68-i*0.045,0.56,0.54),green if i%2==0 else lime)
	...
	return body
static func animate(root: Node3D, kind: String, time: float, velocity: Vector3, stunned: bool, pinned: bool) -> void:
	...
	match kind:
		"HAIRBALL":
			...
			if tongue: tongue.rotation.x = sin(time*5.0)*0.16
		"PLATELET":
			var helmet := root.get_node_or_null("HelmetDome") as Node3D
			if helmet: helmet.rotation.z = sin(time*5.2)*0.05
		"PARASITE":
			var idx := 0
			for child in root.get_children():
