...
func _animate_role_model() -> void:
	...
	var beat: float = sin(living_time * (7.5 + move_ratio * 2.5))
	...
	character_visual.position.y = beat * 0.035 * move_ratio
	character_visual.rotation.z = beat * 0.025 * move_ratio
	var cap := character_visual.get_node_or_null("ShroomCap") as MeshInstance3D
	if cap: cap.rotation.z = sin(living_time * 3.6) * 0.055
	var prong_l := character_visual.get_node_or_null("SparkProngL") as MeshInstance3D
	var prong_r := character_visual.get_node_or_null("SparkProngR") as MeshInstance3D
	if prong_l: prong_l.rotation.z = sin(living_time * 5.2) * 0.06
	if prong_r: prong_r.rotation.z = -sin(living_time * 5.2) * 0.06
...
func _tick_enemies(delta: float) -> void:
	for e in enemies:
		...
		if hpl:
			...
			hpl.text = "%s%s\n%d/%d" % [String(e.get_meta("kind")), ctag, int(e.get_meta("hp")), int(e.get_meta("max_hp"))]
