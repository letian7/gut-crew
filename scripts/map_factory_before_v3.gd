...
static func _build_shell(root: Node3D) -> void:
	...
	mesh.height = 2.0
	shell.mesh = mesh
	shell.position = Vector3(0, 3.4, 0)
	shell.scale = Vector3(15.8, 7.4, 10.2)
	shell.material_override = mat(Color("#7f3159"), 1.0, 0.0, true)
	root.add_child(shell)
	var ceiling := sphere(root, "CeilingBulge", Vector3(0, 5.7, 0.2), Vector3(14.5, 2.6, 8.4), Color("#9c466c"))
