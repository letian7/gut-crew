...
static func spawn_hit(parent: Node3D, pos: Vector3, color: Color) -> void:
	var ring := torus(parent, "HitRing", pos, Vector3.ONE * 0.22, color, 0.72, 3.2)
	var tw := parent.create_tween()
	tw.tween_property(ring, "scale", Vector3.ONE * 1.25, 0.16)
	tw.tween_property(ring, "scale", Vector3.ZERO, 0.08)
	tw.tween_callback(ring.queue_free)
	_burst(parent, pos, color, 7, 0.9)
static func _spark(parent: Node3D, slot: int, origin: Vector3, forward: Vector3, target: Vector3) -> void:
	...
	if slot == 0:
		...
		for i in range(3):
			var ring := torus(parent, "OverloadRing", origin, Vector3.ONE * (0.25 + i * 0.10), yellow, 0.52, 3.6)
			var tw := parent.create_tween()
			tw.tween_property(ring, "scale", Vector3.ONE * (2.2 + i * 0.9), 0.24 + i * 0.05)
			tw.tween_property(ring, "scale", Vector3.ZERO, 0.10)
			tw.tween_callback(ring.queue_free)
		_burst(parent, origin + Vector3.UP * 0.2, white, 14, 2.4)
static func _kaka(parent: Node3D, slot: int, origin: Vector3, forward: Vector3, target: Vector3) -> void:
	...
	if slot == 0:
		...
		for i in range(6):
			...
			var spike := cylinder(root, "BoneSpike", p, p + Vector3.UP * (0.75 + float(i % 3) * 0.18), 0.12 + float(i % 2) * 0.035, bone, 1.0, 0.25)
		...
		_burst(parent, origin + forward * 2.8, orange, 10, 1.4)
