...
static func _build_shell(root: Node3D) -> void:
	...
	mesh.height = 2.0
	shell.mesh = mesh
	shell.position = Vector3(0, 4.35, 0)
	shell.scale = Vector3(21.8, 9.7, 17.2)
	shell.material_override = mat(Color("#713052"), 1.0, 0.08, true)
	root.add_child(shell)
	var ceiling := sphere(root, "CeilingBulge", Vector3(0, 7.95, 0.2), Vector3(14.8, 1.45, 8.7), Color("#9c466c"))
...
static func _build_rugae(root: Node3D) -> void:
	...
	for side in [-1.0, 1.0]:
		for i in range(7):
			...
			var fold := sphere(root, "RugaFold", Vector3(side * 19.15, 2.10 + float(i % 2) * 0.32, z), Vector3(0.34, 1.02, 1.72), pinks[i % pinks.size()])
			fold.rotation.z = side * (0.20 + float(i % 3) * 0.05)
			fold.set_meta("base_scale", fold.scale)
	for i in range(8):
		var x := -10.5 + float(i) * 3.0
		var fold := sphere(root, "BackFold", Vector3(x, 2.25 + float(i % 2) * 0.36, 15.05), Vector3(1.48, 0.84, 0.30), pinks[(i + 1) % pinks.size()])
		fold.set_meta("base_scale", fold.scale)
...
static func animate(root: Node3D, time: float, danger: float = 0.0) -> void:
	...
	for child in root.get_children():
		...
		elif mi.name == "MucusStrand":
			var base: Vector3 = mi.get_meta("base_scale", mi.scale)
			var phase: float = float(mi.get_meta("phase", 0.0))
			mi.scale = Vector3(base.x, base.y * (1.0 + sin(time*1.8+phase)*0.09), base.z)
...
static func _build_platform_skins(root: Node3D) -> void:
	var skin_a := sphere(root, "HighGroundSkin", Vector3(-8.2, 0.92, 3.8), Vector3(2.55, 0.32, 1.55), Color("#bd6682"))
	skin_a.set_meta("base_scale", skin_a.scale)
	var skin_b := sphere(root, "HighGroundSkin", Vector3(7.7, 0.98, 4.0), Vector3(2.35, 0.34, 1.42), Color("#aa5577"))
...
static func _build_hanging_folds(root: Node3D) -> void:
	for i in range(10):
		...
		var fold := sphere(root, "HangingFold", Vector3(x,6.25,z), Vector3(0.58,1.28 + float(i%3)*0.20,0.76), Color("#b95e7d"))
...
static func enhance_v4(root: Node3D) -> void:
	_build_muscle_fibers(root)
	_build_mucus_strands(root)
	_build_tissue_seams(root)

static func _build_muscle_fibers(root: Node3D) -> void:
	var deep := Color("#8f385e")
	for i in range(14):
		var x: float = -9.6 + float(i) * 1.48
		var y: float = 2.0 + float(i % 3) * 0.42
		var fiber := cylinder(root,"MuscleFiber",Vector3(x,y,-7.42),Vector3(0.08,0.95+float(i%2)*0.24,0.08),deep,Vector3(0.0,0.0,0.72+sin(float(i))*0.16),0.10)
		fiber.set_meta("phase",float(i)*0.21)
	for i in range(8):
		var z: float = -4.8 + float(i) * 1.25
		cylinder(root,"SideFiber",Vector3(-11.85,2.1+float(i%2)*0.55,z),Vector3(0.06,0.78,0.06),Color("#a8436a"),Vector3(0.28,0.0,0.22),0.08)

static func _build_mucus_strands(root: Node3D) -> void:
	for i in range(8):
		var x: float = -7.7 + float(i) * 2.15
		var z: float = -2.7 + sin(float(i)*1.3) * 2.2
		var strand := sphere(root,"MucusStrand",Vector3(x,4.45,z),Vector3(0.075,1.05+float(i%3)*0.18,0.075),Color("#f889c8"),0.26,0.20)
		strand.set_meta("base_scale",strand.scale)
		strand.set_meta("phase",float(i)*0.46)
		sphere(root,"MucusDrop",Vector3(x,3.38-float(i%3)*0.17,z),Vector3(0.14,0.20,0.14),Color("#ff9dd3"),0.38,0.25)

static func _build_tissue_seams(root: Node3D) -> void:
	for side in [-1.0,1.0]:
		for i in range(7):
			var z: float = -4.3 + float(i) * 1.35
			var knot := sphere(root,"TissueSeam",Vector3(side*10.9,0.48,z),Vector3(0.24,0.11,0.18),Color("#d46083"),1.0,0.06)
			knot.rotation.y = float(i) * 0.21
