...
func _build_hud() -> void:
	...
	help_label.text = "WASD move | Shift sprint | Space jump | RMB/Ctrl dodge | Q/E skills | F/LMB interact | Tab view | Gamepad supported"
...
func _update_hud() -> void:
	...
	status_label.text = "%s | HP %d | CREDITS %d | Q %s %s | E %s %s | KOs %d | SYNC %d BEST x%d%s" % [ROLE_NAMES[role_index], int(hp), credits, skills[0], qtxt, skills[1], etxt, defeats, synergies, combo_best, payload_txt]
...
func _apply_role_secondary_motion(clay_tick: float, idle: float) -> void:
	...
	if role_index == 0:
		...
		if pl: pl.rotation.z = sin(clay_tick * 5.2) * 0.065
		if pr: pr.rotation.z = -sin(clay_tick * 5.2) * 0.065
	elif role_index == 2:
	...
	match role_index:
		0:
			for n in ["SparkProngL","SparkProngR"]:
				var pr := _part(n); if pr: pr.scale.y = 1.0 + strength * 0.18
		1:
...
func _physics_process(delta: float) -> void:
	...
	if drink_time > 0.0 and drink_wave and absf(player.position.x - drink_wave.position.x) < 1.3: player.velocity.x += drink_dir * (21.0 if acid_umbrella_time > 0.0 else 15.0) * delta
...
func _damage_enemy(e: CharacterBody3D, amount: float, stun := 0.0, pin := 0.0, impulse := Vector3.ZERO) -> void:
	...
	if e != mouse_target:
		...
		if combo_t > 0.0 and last_role >= 0 and last_role != role_index:
			...
			_toast("SYNC x%d  %s + %s" % [chain, ROLE_NAMES[last_role], ROLE_NAMES[role_index]], Color("#ffdd65"), 2.0)
...
func _tick_enemies(delta: float) -> void:
	for e in enemies:
		...
		if hpl:
			...
			var kind_name := String(e.get_meta("kind"))
			hpl.text = "%s%s%s\n%d/%d" % [kind_name, ctag, attack_tag, int(e.get_meta("hp")), int(e.get_meta("max_hp"))]
			hpl.modulate = (Color(1.0,0.82,0.35) if kind_name == "PLATELET" else (Color(0.86,0.55,1.0) if kind_name == "HAIRBALL" else Color(0.65,1.0,0.48))) if attack_windup > 0.0 else Color.WHITE
		...
		if old_attack > 0.0 and attack_windup <= 0.0:
		...
		elif old_attack <= 0.0 and attack_cd <= 0.0 and dist_to_player < 1.65 and stun <= 0.0 and pinned <= 0.0:
			attack_windup = 0.36
			attack_cd = 1.15
...
func _update_living_events(delta: float) -> void:
	...
	if in_acid and tide > 0.22 and not on_high_ground:
		hp = maxf(0.0, hp - 11.0 * delta * (0.18 if acid_umbrella_time > 0.0 else 1.0))
...
func _skill_spark(slot: int) -> void:
	if slot == 0:
		...
		if reacted:
			synergies += 1
			_toast("SYNC: OVERCHARGED BIO ZONE", Color("#ffe15e"), 2.0)
...
func _tick_mission_mouse(e: CharacterBody3D, delta: float) -> void:
	...
	var hit_wall := false
	if e.position.x < -13.3 or e.position.x > 13.3:
		e.velocity.x *= -0.72
		hit_wall = true
	if e.position.z < -7.6 or e.position.z > 7.6:
		e.velocity.z *= -0.72
		hit_wall = true
	if hit_wall:
		wall_bump = 0.22
...
func _buy_item(index: int) -> bool:
	...
	match index:
		0:
			...
			_toast("ACID UMBRELLA: acid -82%, swallow push +40%", Color("#ffe36b"), 2.8)
		1:
			plasma_soda_time = maxf(plasma_soda_time, 16.0)
			_toast("PLASMA SODA: speed +28%, super jump", Color("#64e5ff"), 2.5)
		2:
			catnip_time = 14.0
