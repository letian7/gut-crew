...
func _refresh_mission_ui() -> void:
	...
	match mission_phase:
		...
		"chase": mission_label.text = "CHASE  - control and capture the electronic mouse"
		"return": mission_label.text = "RETURN  - carry the mouse back to the capsule"
...
Time  %.1fs
KOs  %d
Patient stabilized" % [round_time, defeats]
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	interact_down = false
	capture_bar.visible = false
	interact_label.text = ""
